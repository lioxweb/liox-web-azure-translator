 ...
 * Plugin URI: https://lioxweb.com/
 * Description: Adds Microsoft Azure AI Translator as an automatic translation engine for TranslatePress.
 * Version: 1.1.1
 * Author: LioX Web
 * Author URI: https://lioxweb.com/
 * Text Domain: liox-azure-translator
...
define( 'LWX_AZURE_TRANSLATOR_VERSION', '1.1.1' );
...
function lwx_azure_add_engine( $engines ) {
    ...
    $by_value[ LWX_AZURE_ENGINE ] = array(
        ...
        'label' => __( 'Microsoft Azure Translator (LioX Web)', 'liox-azure-translator' ),
    );
...
function lwx_azure_render_settings( $mt_settings ) {
        ...
        if ( is_object( $trp ) && method_exists( $trp, 'get_component' ) ) {
            ...
            if ( is_object( $translator ) && method_exists( $translator, 'check_api_key_validity' ) ) {
                ...
                if ( ! empty( $check['error'] ) ) {
                    $show_error    = true;
                    $error_message = ! empty( $check['message'] ) ? (string) $check['message'] : __( 'Azure Translator connection failed.', 'liox-azure-translator' );
                }
            }
        }
    }
    ...
    <div class="trp-engine trp-automatic-translation-engine__container" id="<?php echo esc_attr( LWX_AZURE_ENGINE ); ?>">
        <span class="trp-primary-text-bold"><?php esc_html_e( 'Azure Translator API Key', 'liox-azure-translator' ); ?></span>
        <div class="trp-automatic-translation-api-key-container">
            <input
                ...
                placeholder="<?php esc_attr_e( 'Paste Key 1 or Key 2 from Azure', 'liox-azure-translator' ); ?>"
            />
            <?php
        ...
        <span class="trp-description-text">
            <?php esc_html_e( 'Use a key from Azure Portal -> Translator resource -> Keys and Endpoint.', 'liox-azure-translator' ); ?>
        </span>

        <div class="trp-deepl-settings__container" style="margin-top:16px;">
            <span class="trp-primary-text-bold"><?php esc_html_e( 'Azure Region', 'liox-azure-translator' ); ?></span>
            <input
                type="text"
            ...
            />
            <span class="trp-description-text">
                <?php esc_html_e( 'Required for regional or multi-service resources. Leave blank for a global single-service Translator resource.', 'liox-azure-translator' ); ?>
            </span>
        </div>

        <div class="trp-deepl-settings__container" style="margin-top:16px;">
            <span class="trp-primary-text-bold"><?php esc_html_e( 'Azure Endpoint', 'liox-azure-translator' ); ?></span>
            <input
                type="url"
                class="trp-text-input"
            ...
            />
            <span class="trp-description-text">
                <?php esc_html_e( 'Normally leave the default. Paste a custom Azure endpoint only if your resource requires one.', 'liox-azure-translator' ); ?>
            </span>
        </div>

        <div class="trp-deepl-settings__container" style="margin-top:16px;">
            <span class="trp-primary-text-bold"><?php esc_html_e( 'Azure Target Languages (optional)', 'liox-azure-translator' ); ?></span>
            <input
                type="text"
                class="trp-text-input"
            ...
            />
            <span class="trp-description-text">
                <?php esc_html_e( 'Comma-separated Azure language codes. Leave blank to allow every supported target language, including English. If you use a list and want new English strings translated too, include en. Example: en,fr,de,it,es,nl.', 'liox-azure-translator' ); ?>
            </span>
        </div>

        <div class="trp-deepl-settings__container" style="margin-top:16px;">
            <span class="trp-primary-text-bold"><?php esc_html_e( 'Azure Monthly Hard Limit (characters)', 'liox-azure-translator' ); ?></span>
            <input
                type="number"
                min="0"
                step="1000"
            ...
            />
            <span class="trp-description-text">
                <?php esc_html_e( 'LioX Web safety cap for Azure requests. Default: 1,900,000 characters per calendar month. Set 0 to disable this plugin-level cap.', 'liox-azure-translator' ); ?>
            </span>
            <span class="trp-description-text" style="display:block;margin-top:6px;">
                <strong><?php esc_html_e( 'Current month:', 'liox-azure-translator' ); ?></strong>
                <?php
                if ( 0 === $monthly_limit ) {
                    printf(
                        /* translators: %s: used character count. */
                        esc_html__( '%s characters tracked — hard limit disabled.', 'liox-azure-translator' ),
                        esc_html( number_format_i18n( $monthly_used ) )
                    );
                } else {
                    printf(
                        /* translators: 1: used characters, 2: monthly limit, 3: remaining characters. */
                        esc_html__( '%1$s / %2$s used — %3$s remaining.', 'liox-azure-translator' ),
                        esc_html( number_format_i18n( $monthly_used ) ),
            ...
            <span class="trp-description-text" style="display:block;margin-top:4px;">
                <?php esc_html_e( 'The local counter resets automatically when the UTC calendar month changes. It counts Azure translation requests sent by this WordPress installation, including explicit API credential tests.', 'liox-azure-translator' ); ?>
            </span>
        </div>
    </div>
    <?php
...
function lwx_azure_dependency_notice() {
    ...
    <div class="notice notice-warning">
        <p>
            <strong><?php esc_html_e( 'LioX Web Azure Translator:', 'liox-azure-translator' ); ?></strong>
            <?php esc_html_e( 'TranslatePress must be installed and active for this plugin to work.', 'liox-azure-translator' ); ?>
        </p>
    </div>
    <?php
}
